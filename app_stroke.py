...
st.markdown("""
    ...
    * { font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    /* Status Colors */
    .status-box { padding: 20px; border-radius: 12px; margin: 15px 0; font-weight: 600; text-align: center; }
    .high { background-color: #ffe5e5; color: #d90429; border: 1px solid #ffb3b3; }
    .low { background-color: #e5f9e0; color: #2d6a4f; border: 1px solid #b7e4c7; }
    .warn { background-color: #fff4e5; color: #d68102; border: 1px solid #ffe5b3; }

    /* Buttons */
    .stButton>button {
        border-radius: 12px; font-weight: 600; padding: 0.6rem 2rem;
...
if not st.session_state.nama_dokter:
    ...
    with col_m:
        st.markdown("""
            <div class='main-card'>
                <h1 style='text-align: center; color: #004a99; margin-bottom: 5px;'>🩺 SINTALA-STROKE</h1>
                <p style='text-align: center; color: #6c757d; margin-bottom: 30px;'>Sistem Informasi & Analisa Stroke Terpadu</p>
            </div>
        """, unsafe_allow_html=True)
        input_dr = st.text_input("Nama Lengkap Dokter (beserta gelar):", placeholder="Contoh: dr. Faisal Bayu, Sp.N")
        if st.button("Masuk Ke Dashboard", use_container_width=True):
            if input_dr:
                st.session_state.nama_dokter = input_dr
                st.rerun()
            else: st.error("Harap masukkan nama Dokter.")
    st.stop()

# --- 3. DASHBOARD UTAMA (PILIHAN 3 MODUL) ---
if st.session_state.pilihan_layanan is None:
    st.markdown(f"## Selamat Datang, {st.session_state.nama_dokter}")
    st.write("Silakan pilih modul pemeriksaan klinis di bawah ini:")
    
    c1, c2, c3 = st.columns(3)
    with c1:
        st.markdown('<div class="module-card"><h3>📊 FSRP</h3><p>Framingham Stroke Risk Profile<br>Skrining Risiko 10 Tahun</p></div>', unsafe_allow_html=True)
        if st.button("Buka FSRP", use_container_width=True):
            st.session_state.pilihan_layanan = "FSRP"; st.rerun()
    with c2:
        st.markdown('<div class="module-card" style="border-top-color: #dc3545;"><h3>🚨 NIHSS</h3><p>National Institutes of Health<br>Stroke Scale (11 Poin Lengkap)</p></div>', unsafe_allow_html=True)
        if st.button("Buka NIHSS", use_container_width=True):
            st.session_state.pilihan_layanan = "NIHSS"; st.rerun()
    with c3:
        st.markdown('<div class="module-card" style="border-top-color: #ff8c00;"><h3>🧠 SIRIRAJ</h3><p>Siriraj Stroke Score<br>Pembeda Infark vs Hemoragik</p></div>', unsafe_allow_html=True)
...
# --- 4. SIDEBAR ---
with st.sidebar:
    st.markdown(f"<h3>SINTALA</h3>", unsafe_allow_html=True)
    st.write(f"DPJP: **{st.session_state.nama_dokter}**")
    st.caption(f"Modul Aktif: {st.session_state.pilihan_layanan}")
...
if st.session_state.pilihan_layanan == "FSRP":
    ...
    with st.form("fsrp_form"):
        ...
        with col2:
            ...
            jantung = st.multiselect("Penyakit Jantung", ["PJK", "AF", "LVH (EKG)"])
        submit = st.form_submit_button("HITUNG RISIKO")
    
    if submit:
        poin = sum([u55*2, (tds>=140)*3, smk*3, dm*2, ("PJK" in jantung)*2, ("AF" in jantung)*4, ("LVH (EKG)" in jantung)*5])
        kat = "TINGGI" if poin >= 6 else "RENDAH"
        st.markdown(f"<div class='report-box print-container'><h3>HASIL FSRP</h3><p>Pasien: {nama_p}</p><h2>SKOR: {poin} (RISIKO {kat})</h2><p style='text-align:right;'>{st.session_state.nama_dokter}</p></div>", unsafe_allow_html=True)
